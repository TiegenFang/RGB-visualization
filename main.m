...
% 获取文件夹中所有图片文件（假设是常见的图片格式，如 .jpg 或 .png）
image_files = dir(fullfile(input_folder, '*.bmp'));  % 或者使用 '*.png' 根据需要修改
...
%         [processed_image, all_T_new] = temp(processed_image, r_b,r_g,g_b,i,Tpre,all_T_new);  % 计算燃烧区域温度
        [processed_image,T_ig] = temp(processed_image, r_b,r_g,g_b,i,Tpre,output_folder,T_ig);  % 计算燃烧区域温度
